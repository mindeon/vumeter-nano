...
#define SAMPLING_FREQ 32768                         // Hz, must be 40000 or less due to ADC conversion time. Determines maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
#define AMPLITUDE 256                              // Depending on your audio source level, you may need to alter this value. Can be used as a 'sensitivity' control.
#define AUDIO_IN_PIN A0                            // Signal in on this pin huzzah32
...
void loop()
{
  ...
  { // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    ...
    { // Add a crude noise filter
      ...
      if (i > 5 && i <= 7) bandValues[3] += (int)vReal[i];
      if (i>7   && i<=9  ) bandValues[4]  += (int)vReal[i];
      if (i>9   && i<=11 ) bandValues[5]  += (int)vReal[i];
      if (i>11  && i<=13 ) bandValues[6]  += (int)vReal[i];
      if (i>13  && i<=15 ) bandValues[7]  += (int)vReal[i];
      if (i>15  && i<=17 ) bandValues[8]  += (int)vReal[i];
      if (i>17  && i<=19 ) bandValues[9]  += (int)vReal[i];
      if (i>19  && i<=21 ) bandValues[10] += (int)vReal[i];
      if (i>21  && i<=23 ) bandValues[11] += (int)vReal[i];
      if (i>23  && i<=25) bandValues[12] += (int)vReal[i];
      if (i>25 && i<=27) bandValues[13] += (int)vReal[i];
      if (i>27 && i<=29) bandValues[14] += (int)vReal[i];
      if (i>29          ) bandValues[15] += (int)vReal[i];
    }
  }
...
void rainbowBars(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    {
      matrix->drawPixel(y, x, CHSV((y / BAR_WIDTH) * (255 / NUM_BANDS), 255, 255));
    }
  }
}
...
void purpleBars(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    for (int y = TOP; y >= TOP - barHeight; y--)
    {
      matrix->drawPixel(y, x, ColorFromPalette(purplePal, x * (255 / (barHeight + 1))));
    }
  }
}
...
void changingBars(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    for (int y = TOP; y >= TOP - barHeight; y--)
    {
      matrix->drawPixel(y, x, CHSV(x * (255 / kMatrixHeight) + colorTimer, 255, 255));
    }
  }
}
...
void centerBars(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    for (int y = yStart; y <= (yStart + barHeight); y++)
    {
      ...
      matrix->drawPixel(y, x, ColorFromPalette(heatPal, colorIndex));
    }
  }
}
...
void whitePeak(int band)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    matrix->drawPixel(peakHeight, x, CHSV(255, 255, 255)); // white: CHSV(0,0,255)
  }
}
...
void outrunPeak(int band)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    matrix->drawPixel(peakHeight, x, ColorFromPalette(outrunPal, peakHeight * (255 / kMatrixHeight)));
  }
}
...
void waterfall(int band)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    matrix->drawPixel(0, x, CHSV(constrain(map(bandValues[band], 0, highestBandValue, 160, 0), 0, 160), 255, 255));
  }
  ...
  if (band == NUM_BANDS - 1)
  {
    for (int y = kMatrixHeight - 2; y >= 0; y--)
    {
      ...
      {
        int pixelIndexY = matrix->XY(y + 1, x);
        int pixelIndex = matrix->XY(y, x);
